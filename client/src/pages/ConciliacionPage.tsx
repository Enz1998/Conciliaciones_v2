...
type Tab = 'summary' | 'unmatched' | 'matched';
...
export function ConciliacionPage({ id, onBack }: Props) {
  ...
  const extUnmatched: NormalizedMovement[] = sortMovs(
    (data.extractoMovs || []).filter((m: NormalizedMovement) => m.match_type === 'UNMATCHED' && m.match_type !== 'TAX_CHILD'),
    extSort
  );
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 28 }}>
        ...
        <div>
          <h2 style={{ fontSize: 22, fontWeight: 700, color: '#111', margin: 0 }}>{data.nombre}</h2>
          <span style={{ fontSize: 12, color: '#888' }}>{data.estado}</span>
        </div>
        <button
          ...
          style={{
            ...
            background: rematching ? '#e3f2fd' : '#1565c0',
            color: rematching ? '#1565c0' : '#fff',
            fontSize: 13,
            fontWeight: 600,
      ...
      <div style={{ display: 'flex', gap: 0, marginBottom: 24, borderBottom: '1px solid #eee' }}>
        {([
          ...
          >
            {label}
            {t === 'unmatched' && <span style={{ marginLeft: 6, fontSize: 11, color: '#999' }}>({extUnmatched.length + mayUnmatched.length})</span>}
            {t === 'matched' && <span style={{ marginLeft: 6, fontSize: 11, color: '#999' }}>({data.matches?.length || 0})</span>}
          </button>
        ))}
      </div>
        ...
        <div>
          {selected.size > 0 && (
            <div style={{
              ...
              <button style={{ ...s.matchBtn, background: '#fff', color: '#111' }} onClick={handleManualMatch}>🔗 Matchear</button>
            </div>
          )}
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            {/* Columna Extracto */}
            <div style={{ border: '1px solid #eee', borderRadius: 10, overflow: 'hidden' }}>
              <div style={{ padding: '10px 16px', background: '#f8f9fb', borderBottom: '1px solid #eee', fontSize: 12, fontWeight: 600, color: '#1565c0' }}>
                🏦 Extracto Bancario — {extUnmatched.length} sin match
              </div>
                ...
                <table style={s.table}>
                  ...
                  <tbody>
                    ...
                    {bankCharges.length > 0 && (
                      <>
                        <tr>
                          <td colSpan={5} style={{ padding: '12px 16px', background: '#f5f5f5', borderBottom: '1px solid #eee', fontSize: 12, fontWeight: 600, color: '#555' }}>
                            📌 Cargos Bancarios ({bankCharges.length} items)
                          </td>
                        </tr>
                    ...
                    {extUnmatched.length === 0 && (
                      <tr><td colSpan={5} style={{ padding: 40, textAlign: 'center', color: '#aaa', fontSize: 13 }}>Todo matcheado ✓</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
            ...
            {/* Columna Mayor */}
            <div style={{ border: '1px solid #eee', borderRadius: 10, overflow: 'hidden' }}>
              <div style={{ padding: '10px 16px', background: '#f8f9fb', borderBottom: '1px solid #eee', fontSize: 12, fontWeight: 600, color: '#c62828' }}>
                📊 Libro Mayor (ERP) — {mayUnmatched.length} sin match
              </div>
              <div style={{ maxHeight: 'calc(100vh - 320px)', overflow: 'auto' }}>
                <table style={s.table}>
                  <thead>
            ...
            <div key={match.id} style={{ border: '1px solid #eee', borderRadius: 8, overflow: 'hidden' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 14px', background: '#fafafa', borderBottom: '1px solid #eee', fontSize: 12 }}>
                ...
                {match.match_type !== 'TAX_CHILD' && (
                  <button onClick={() => handleUnmatch(match.id)} style={{ marginLeft: 'auto', padding: '3px 10px', borderRadius: 6, border: '1px solid #ddd', background: '#fff', color: '#b91c1c', fontSize: 11, cursor: 'pointer', fontWeight: 500 }}>
                    ✕ Deshacer
                  </button>
                )}
              </div>
...
function StatCard({ label, value, color }: { label: string; value: string; color: string }) {
  return (
    <div style={{ padding: '16px 20px', borderRadius: 10, background: '#fff', border: '1px solid #f0f0f0' }}>
      <div style={{ fontSize: 11, color: '#999', textTransform: 'uppercase', fontWeight: 600, letterSpacing: 0.3, marginBottom: 4 }}>{label}</div>
